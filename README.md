...
에이전트는 등록된 볼트와 주제 신호를 확인해 대상을 정합니다. 대상이 모호하면 실행 전에 질문합니다.
...
원격 토큰은 레지스트리·git·`publish.json`에 저장하지 않고 환경 변수 또는 설정 디렉터리 `secrets.json`(`0600`)에만 둡니다. 발행은 단방향(local→원격)이며 원격→local 역방향이나 원격 페이지 삭제는 하지 않습니다.

민감한 자료를 다루는 볼트라면 파일 권한, 디스크 암호화, 원격 저장소 접근 제어를 별도로 설정하세요. 볼트로 `cd`한 뒤에는 그 볼트 `CLAUDE.md`의 보안 원칙이 적용됩니다.
