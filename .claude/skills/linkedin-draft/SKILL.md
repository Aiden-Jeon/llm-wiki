...
대상 볼트는 라우팅으로 해소한다(`WIKI-CLI.md`). 기본은 커리어 자료를 가진 `open` 볼트다. `kind: secure` 볼트에서 raw 민감 자료를 직접 끌어오지 않는다 — 이미 익명화된 커리어 페이지만 사용한다.
...
- 회사 프로젝트를 언급하려면 커리어 엔티티에 이미 익명화된 형태만 쓴다. `kind: secure` 볼트에서 raw 자료를 끌어오지 않는다.
